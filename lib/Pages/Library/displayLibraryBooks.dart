...
class DisplayLibraryBooks extends StatelessWidget {
  final MyLibraryBloc bloc;

  const DisplayLibraryBooks({Key key, this.bloc}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<MyLibraryBloc, MyLibraryState>(
      cubit: bloc,
      builder: (BuildContext context, MyLibraryState state) {
        if (state is LibraryBooksLoading) {
          return Container(
            child: Center(
        ...
        if (state is LibraryBooksNotLoaded) {
          return Text("${state.errors}");
        }

        if (state is LibraryBooksLoaded) {
          final List<Books> currents = state.library;
          return Container(
